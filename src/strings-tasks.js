...
function invertCase(str) {
  return str.split('').reduce((out, el) => {
    let accum = out;
    if (el.toUpperCase() === el) {
      accum = out.concat(el.toLowerCase());
    } else {
      accum = out.concat(el.toUpperCase());
    }
...
function extractNameFromTemplate(value) {
  ...
  return `${strArr[1]} ${strArr[2].replaceAll('!', '')}`;
}
...
function encodeToRot13(str) {
  ...
  let output = '';
  str.split('').forEach((el) => {
    const curIndex = arrInput.indexOf(el);
    if (curIndex === -1) {
      output = output.concat(el);
    } else {
      output = output.concat(arrOutput[curIndex]);
    }
  });
